...
(async () => {
  ...
  const scrapeData = async (page, url) => {
    ...
    const data = await page.evaluate(() => {
      ...
      document.querySelectorAll('.scheme').forEach((schemeElement) => {
        ...
        schemes.push({ title, date, beneficiary, benefits, howToApply });
      });
      return schemes;
    });
    const resultWithUUID = data.map((it)=>({id:uuidv4(),scheme_url:url,...it}))
  ...
  fs.writeFileSync('manipur.json', JSON.stringify(allSchemes, null, 2), 'utf-8');
